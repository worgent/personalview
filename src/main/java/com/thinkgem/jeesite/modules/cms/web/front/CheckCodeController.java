...
import com.thinkgem.jeesite.modules.cms.utils.MathUtil;
import com.thinkgem.jeesite.modules.cms.utils.SendMsg_webchinese;
...
public class CheckCodeController {
    ...
    JSONObject getSMsCode(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if( type != null &&
                type.isEmpty() == false &&
                type.equals(SessionAttributeEnum.REGIST_USER_CODE.getKey()) ){
            //判断短信发送间隔
            lastSendTime = MathUtil.parseLong((session.getAttribute(SessionAttributeEnum.REGIST_USER_CODE_TIME.getKey())));
           // lastPhone = session.getAttribute(SessionAttributeEnum.REGIST_USER_PHONE.getKey()).toString();
            msg += rand;
            msg += "（动态验证码），您于";
        ...
        //i
        if( lastSendTime + CmsConstants.sendMsgInterval > TimeUtil.now() ){
        ...
        if( type != null &&
                type.isEmpty() == false &&
                type.equals(SessionAttributeEnum.REGIST_USER_CODE.getKey()) ){
            session.setAttribute(SessionAttributeEnum.REGIST_USER_CODE.getKey(),
                    rand);
            session.setAttribute(SessionAttributeEnum.REGIST_USER_PHONE.getKey(),
                    phone);
            session.setAttribute(SessionAttributeEnum.REGIST_USER_CODE_TIME.getKey(),
                    TimeUtil.now());
        } else {
...
//                    break;
        ...
        HttpSession session = request.getSession(true);
        log.error("session key," + session.getAttribute(SessionAttributeEnum.REGIST_USER_CODE.getKey()));
...
//            if (type.trim().equals(SessionAttributeEnum.REGIST_USER_CODE.getKey())) {
//                log.info("获取验证码的来源是："+type);
//                session.setAttribute(SessionAttributeEnum.REGIST_USER_CODE.getKey(), sRand);
//                session.setAttribute("jiangrushe", sRand);
//                session.setAttribute("wanglihuan", "jiangrushe");
//                log.info("从seesion中后去验证码"+request.getSession().getAttribute(SessionAttributeEnum.REGIST_USER_CODE.getKey()));
//            } else {
