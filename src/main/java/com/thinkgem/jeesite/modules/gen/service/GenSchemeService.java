...
//	private GenTemplateDao genTemplateDao;
	...
	private String generateCode(GenScheme genScheme){
		...
		//如果有父模板，添加自己的父模板
		//todo 扩展为多父模板
		if( genTable.getParentTable().isEmpty() == false &&
				genTable.getParentTableFk().isEmpty() == false){
			GenTable parentTable = new GenTable();
			parentTable.setName(genTable.getParentTable());
			List<GenTable> parentTables = genTableDao.findList(parentTable);
			if( parentTables.size() == 1 ){
				genTable.setParent(parentTables.get(0));
			}
		}
